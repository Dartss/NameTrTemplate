...
/**
 * This class is designed to communicate with the Jedis API build upon Jedis
 * java library.
...
public class JedisHelper
{
	...
	if (jedisHelper.jedisPool == null || jedisHelper.jedisPool.isClosed())
	{
	    ...
	    // jedisHelper.jedisPool = jedisHelper.getPool(jedisHelper.host,
	    // jedisHelper.port, jedisHelper.config);
	} else
	{
     ...
     * Note: this is actually a string operation, that is, in Redis there are
     * not "integer" types.
     * Simply the string stored at the key is parsed as a base 10 64 bit signed
     * integer, incremented,
